...
if(length(args) != 0) {
    ...
    if(length(args) > 0) {
        ...
        stop("Need two date arguments and one name argument")
    }
} else {
    start <- "15/02/2020"
    end <- "23/03/2020"
...
## save outputs
if(dir.exists(paste0("../outputs", outnm))) {
    stop(paste0("../outputs", outnm, " already exists. Please delete if you wish to overwrite this folder."))
}
...
system(paste0("cp Local_Authority_Districts_\\(December_2019\\)_Boundaries_UK_BUC.zip ../outputs", outnm))
