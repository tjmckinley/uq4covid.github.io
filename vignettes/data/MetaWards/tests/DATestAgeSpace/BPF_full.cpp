...
List BPF_cpp (arma::vec pars, arma::mat C1, arma::mat C2, int lockdown_day,
    ...
    List u2_comb, arma::vec playprobs, arma::ivec ncohorts2, 
    arma::uword tstart, arma::uword tstop, arma::uword npart, int niter, 
    double a1, double a2, double b1, double b2, arma::cube a_dis, arma::cube b_dis,
    double sigma2_age_lad,
    ...
    for(i = 0; i < npart; i++) {
        arma::icube tempmat = as<arma::icube>(u1_comb[i]);
        u1[i] = tempmat;
        u1_new[i] = u1[i];
        tempmat = as<arma::icube>(u2_comb[i]);
        u2[i] = tempmat;
        u2_new[i] = u2[i];
    }
    ...
    for(i = 0; i < npart; i++) {
        mu_night_age_lad[i] = arma::cube(5, nages, ndeathlads); mu_night_age_lad[i].zeros();
        mu_night_age_lad1[i] = arma::cube(5, nages, ndeathlads); mu_night_age_lad1[i].zeros();
    }
...
//                            mu_night_age_lad[i](4, j, k) = (double) mu_night_age_lad[i](4, j, k);
                ...
                // apply aggregation error
                u_night_age_lad.zeros();
                for(l = 0; l < ndeathlads; l++) {
                    for(j = 0; j < nages; j++) {
                        // sample observations
                        u_night_age_lad(0, j, l) = rdtnorm_cpp(
            ...
            if(writeExt == 0) { 
                if(saveAll == 1) {
                    out[i] = List::create(Named("age_lads") = u_night_age_lad);
                } else {
                    out[i] = List::create(Named("full") = u_night_full, _["age_lads"] = u_night_age_lad);
                }
            } else {
                if(saveAll == 2) {
    ...
    for(t = tstart; t < tstop; t++) {
        ...
        // loop over particles
#ifdef _OPENMP
#pragma omp parallel for default(none) private(j, l, k) shared(seeds, npart, nages, nclasses, nlads, C1, C2, u1_moves, u1, u1_new, ncohorts1, u2, u2_new, playprobs, ncohorts2, t, pars, weights, a_dis, b_dis, a1, a2, b1, b2, obsInc_age_lad, PF, tstart, tstop, ndeathlads, lookup, sigma2_age_lad, mu_night_age_lad, lockdown_day)
#endif
        for(i = 0; i < npart; i++) {
        ...
        if(PF == 1) {
            ...
            if(!arma::is_finite(ll)) {
                ...
                if(saveAll == 0 || writeExt == 1) {
                    ...
                    if(tstart != 0) {
                        IntegerVector idx = seq_len(out.length()) - 1;
                        out = out[idx >= npart];
                    }
            ...
            if(niter > 0) {
#ifdef _OPENMP
#pragma omp parallel for default(none) private(j, l, k) shared(seeds, npart, nages, nclasses, nlads, C1, C2, ncohorts1, u1_moves, u1, u1_new, ncohorts2, u2, u2_new, playprobs, t, pars, a_dis, b_dis, a1, a2, b1, b2, obsInc_age_lad, PF, tstart, tstop, ndeathlads, lookup, sigma2_age_lad, condpars, niter, nacc, mu_night_age_lad, mu_night_age_lad1, lockdown_day)
#endif
                for(i = 0; i < npart; i++) {
        ...
        if(saveAll != 0) {
            ...
            for(i = 0; i < npart; i++) {
                ...
                // CAN'T REUSE mu_night_age_lad[i] UNLESS WITHIN A PARTICLE FILTER
                // HENCE WE SIMULATE WHOLE OBSERVATION PROCESS FOR PREDICTIVE
                // DISTRIBUTIONS BELOW
...
//                            mu_night_age_lad[i](4, j, k) = (double) mu_night_age_lad[i](4, j, k);
                ...
                if(writeExt == 0) { 
                    if(saveAll == 1) {
                        out[i + npart * (t - tstart + 1)] = List::create(Named("age_lads") = u_night_age_lad);
                    } else {
                        out[i + npart * (t - tstart + 1)] = List::create(Named("full") = u_night_full, _["age_lads"] = u_night_age_lad);
                    }
                } else {
                    if(saveAll == 2) {
                        std::sprintf(file_name, "%s/p_%u.csv", std::string(outputName[0]).c_str(), i);
    ...
    if(saveAll == 0) {
        ...
        if(snapshot == 1) {
            ...
            if(writeExt == 0) {
                for(i = 0; i < npart; i++) {
                    snap[i] = List::create(Named("u1") = u1[i], _["u2"] = u2[i]);
                }
                snap[npart] = List::create(Named("u1_moves") = u1_moves, _["playprobs"] = playprobs, _["ncohorts1"] = ncohorts1, _["ncohorts2"] = ncohorts2);
            } else {
                for(i = 0; i < npart; i++) {
                    std::sprintf(file_name, "%s/snapshot_u1_t%u_%u.csv", std::string(outputName[0]).c_str(), tstop, i);
                    file.open(file_name);
                    file << "class, ";
                    for(j = 0; j < nages; j++) file << "age" << j + 1 << ", ";
                    file << "cohort\n";
                    for(l = 0; l < u1[i].n_slices; l++) {
                        for(arma::uword r = 0; r < nclasses; r++) {
                            file << r << ", ";
                            for(j = 0; j < nages; j++) {
                                file << u1[i](r, j, l) << ", ";
        ...
        if(writeExt == 0) {
            if(PF == 1) {
                if(snapshot == 1) {
                    return List::create(Named("ll") = ll, _["particles"] = out, _["snapshot"] = snap);
                } else {
                    return List::create(Named("ll") = ll, _["particles"] = out);
                }
            } else {
                if(snapshot == 1) {
                    return List::create(Named("particles") = out, _["snapshot"] = snap);
                } else {
