...
// [[Rcpp::export]]
List APF3_cpp (arma::vec pars, arma::mat C, arma::imat data, arma::uword nclasses, arma::uword nages, arma::uword nlads, arma::imat u1_moves, arma::ivec ncohorts, 
         arma::icube u1_comb, arma::uword ndays, arma::uword npart, double a1, double a2, double b, double a_dis, 
         double b_dis, int saveAll, int writeExt, int PF, int ncores) {
...
//    for(j = 0; j < condpars.n_elem; j++) Rprintf("cp(%d) = %f\n", j, condpars(j));
...
#pragma omp parallel for default(none) private(j, l) shared(seeds, npart, u1_moves, nages, nclasses, nlads, data, C, u1, u1_new, t, weights, qweights, a_dis, b_dis, a1, a2, b, obsInc, pars)
            ...
            for(i = 0; i < npart; i++) {
                ...
                for(j = 0; j < nages; j++) {
                    ...
                    // probs for conditional sampling
...
//                Rprintf("weightsmed(%d) = %f\n", i, weights(i));
                
//                Rprintf("RAinc(2, 316) = %d\n", RAinc(2, 316));
//                Rprintf("u1_night(2, 2, 316) = %d\n", u1_night(2, 2, 316));
//                
//                Rprintf("tempMD\n");
...
//                Rprintf("u1\n");
//                for(j = 4; j < 6; j++) {
//                    for(l = ncohorts(315); l < ncohorts(316); l++) {
//                        Rprintf("%d ", u1[i](j, 4, l));
//                    }
//                    Rprintf("\n");
//                }
//                Rprintf("\n");
...
//                for(j = 4; j < 6; j++) {
//                    for(l = ncohorts(315); l < ncohorts(316); l++) {
//                        Rprintf("%d ", u1_new[i](j, 4, l));
//                    }
//                    Rprintf("\n");
//                }
//                Rprintf("\n");
//                Rprintf("u1_moves\n");
//                for(l = ncohorts(316); l < ncohorts(317); l++) {
//                    Rprintf("%d ", u1_moves(l, 0));
//                }
//                Rprintf("\n");
...
//                    for(l = 0; l < u1_moves.n_rows; l++) {
//                        int s = 0;
//                        for(int r = 4; r < nclasses; r++) {
//                            s += u1_new[i](r, j, l);
//                        }
//                        if(s > 0) {
//                            Rprintf("u11(%d, %d, %d)\n", i, j, l);
//                            for(int r = 1; r < nclasses; r++) {
//                                Rprintf("%d ", u1_new[i](r, j, l));
//                            }
//                            Rprintf("\n");
//                        }
//                    }
...
#ifdef _OPENMP
#pragma omp parallel for default(none) private(j, l, k) shared(seeds, npart, u1_moves, nages, nclasses, nlads, data, C, N_night, N_day, u1, u1_new, t, pars, a_dis, b_dis, a1, a2, b, obsInc, ncohorts)
#endif
            for(i = 0; i < npart; i++) {
...
//            }
            ...
            // normalise weights
            weights = weights - log_sum_exp(weights, 0);
