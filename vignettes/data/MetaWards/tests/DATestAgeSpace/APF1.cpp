...
// [[Rcpp::export]]
List APF1_cpp (arma::vec pars, arma::mat C, arma::imat data, arma::uword nclasses, arma::uword nages, arma::uword nlads, arma::imat u1_moves, arma::ivec ncohorts, 
            ...
            if(MD == 1) {
                ...
                if(PF == 1) {
                    for(j = 0; j < nages; j++) {
                        for(l = 0; l < nlads; l++) {
                            ...
                            arma::vec tempdens1(u1_night(9, j, l) + 1); tempdens1.zeros();
                            for(int r = 0; r <= u1_night(9, j, l); r++) {
                                // observation error for given incidence
                                tempdens1(r) = ldtskellam_cpp(
                                ...
                                if(!arma::is_finite(tempdens(r)) && tempdens(r) >= 0.0) stop("Error in OE\n");
                                ...
                                if(!arma::is_finite(tempdens(r)) && tempdens(r) >= 0.0) stop("Error in MD\n");
                            }
                            // normalise weights
                            double tempnorm = log_sum_exp(tempdens1, 0);
                            tempdens1 = exp(tempdens1 - tempnorm);
                            tempdens1 = tempdens1 / sum(tempdens1);
                                                    
                            // sample incidence and calculate MD
                            tempMD(11, j, l) = rmultinom_cpp(tempdens1, eng);
                ...
                if(PF == 1) {
                    for(j = 0; j < nages; j++) {
                        for(l = 0; l < nlads; l++) {
                            // calculate unnormalised conditional MD densities
                            arma::vec tempdens1(u1_night(5, j, l) - Hinc(j, l) + 1); tempdens1.zeros();
                            for(int r = 0; r <= (u1_night(5, j, l) - Hinc(j, l)); r++) {
                                // observation error for given incidence
                                tempdens1(r) = ldtskellam_cpp(
                                    obsInc(j * nlads + l) - r,
                                    a1 + b * r,
                            ...
                            // sample incidence and calculate MD
                            tempMD(6, j, l) = rmultinom_cpp(tempdens1, eng);
